...
function checkMobileNumber(phone_number){
	var number = /^[0-9]+$/;
...
//Address validation, check for numbers and letters in address field

function checkAddress(street_line_1){
  	var addr = /^[A-Za-z0-9 _/]*$/;

	
	if(street_line_1.value.match(addr) && street_line_1.value.length >=1){
		return true;	
	}
	else if(street_line_1.value.length == 0) {
		alert("Address: Please enter your address\n");

		street_line_1.focus();
		return false;
	}
	else if(!(street_line_1.value.match(addr))){
	  	alert("Address can be numberic, letters, spaces and slash\n" + "Please enter a valid address");
		street_line_1.focus();
		return false;
	}
	else{
